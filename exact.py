#Reference: this exact.py was refered by "ibackus/sod-shocktube" on github.
import numpy as np
import scipy
...
def sound_speed(gamma, pressure, density, dustFrac=0.):
    """
    Calculate sound speed c, scaled by the dust fraction
    """
    scale = np.sqrt(1 - dustFrac)
    return np.sqrt(gamma * pressure/ density) * scale

#shock tube function, for shock or refraction wave
...
#the reigions of waves and calculation positions
def calculate_regions(pl, ul, rhol, pr, ur, rhor, gamma=1.4, dustFrac=0.):
    ...
    # unless...
    if pl < pr:
        rho1 = rhor #1,5 known; 1 get the bigger
        p1 = pr
        u1 = ur
        rho5 = rhol
        p5 = pl
    ...
    # solve the f equation of post-shock pressure
    p4 = scipy.optimize.fsolve(shock_tube_function, p1, (p1, p5, rho1, rho5, gamma))[0] #p4

    # then compute post-shock density and velocity
    z = (p4 / p5 - 1.)
    c5 = sound_speed(gamma, p5, rho5, dustFrac)
    ...
    u4 = c5 * z / (gamma * fact) #u4
    rho4 = rho5 * (1. + gmfac2 * z) / (1. + gmfac1 * z) #rho4

    # shock speed
    w = c5 * fact

    # compute values at foot of rarefaction, at 3
    p3 = p4
    u3 = u4
